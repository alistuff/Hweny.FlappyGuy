...
namespace Hweny.FlappyGuy.Main
{
    public sealed class GameWindow : Form
    {
        ...
        {
            base.OnKeyDown(e);
            OnWndEventHandler<KeyEventArgs>(e, WndKeyDown);
            //缺省的键盘输入事件
            //如果不指示Handled值或Handled值为false，按Escape键时将退出应用程序。
            if (!e.Handled && e.KeyCode == Keys.Escape)
            {
                WindowApiHelper.PostQuitMessage((uint)WindowsMessage.WM_QUIT);
            }
        }
        ...
        {
            base.OnKeyUp(e);
            OnWndEventHandler<KeyEventArgs>(e, WndKeyUp);
        }
        ...
        private void WndMain()
        {
            ...
            float lastTime = 0L;
            float timeDiff = 0L;
            float lastFpsTime = 0L;
            int fpsTicks = 0;
            bool updateFrame = false;
            float accumulatedTime = 0L;

            if (TargetFps <= 0)
                TargetFps = 60;
            else if (TargetFps >= 1000)
                TargetFps = 1000;

            float FRAME_PERIOD = 1000.0f / TargetFps;
            float FRAME_PERIOD_SECONDS = FRAME_PERIOD / 1000.0f;
            ...
            {
                if (!Created) break;
                if (WindowApiHelper.PeekMessage(out msg, IntPtr.Zero, 0, 0, (uint)PM.REMOVE))
                {
                    WindowApiHelper.TranslateMessage(ref msg);
                    WindowApiHelper.DispatchMessage(ref msg);
                }
                else
                {
                    ...
                    while (accumulatedTime > FRAME_PERIOD)
                    {
                        ...
                        OnWndEventHandler<UpdateFrameEventArgs>(
                            new UpdateFrameEventArgs(gameTime, FRAME_PERIOD_SECONDS), WndUpdate);
                        updateFrame = true;
                    }
                    ...
                    if (updateFrame)
                    {
                        ...
                        OnWndEventHandler<RenderFrameEventArgs>(new RenderFrameEventArgs(g), WndRender);
                        g.Dispose();
                    ...
                    {
                        Console.WriteLine("fps:"+fpsTicks);
                        this.Text = "LI-Games "+fpsTicks;
        ...
        /// 窗体更新事件
        /// </summary>
        public event EventHandler<UpdateFrameEventArgs> WndUpdate;
        /// <summary>
        /// 窗体绘制事件
        /// </summary>
        public event EventHandler<RenderFrameEventArgs> WndRender;
        /// <summary>
        /// 窗体清理事件
        /// </summary>
        ...
        /// 窗体键盘输入事件
        /// </summary>
        public event EventHandler<KeyEventArgs> WndKeyDown;
        /// <summary>
        /// 窗体键盘释放事件
        /// </summary>
        public event EventHandler<KeyEventArgs> WndKeyUp;
        /// <summary>
        /// 窗体鼠标输入事件
        /// </summary>
    ...
    internal sealed class WindowApiHelper
    {
        ...
        [DllImport(DLL_NAME, SetLastError = true)]
        public static extern void PostQuitMessage(uint msg);
    }
